...
plot_categorised_open_responses <- function(data, title){
  ...
  data <- data %>%
    mutate(Categories = as.character(Categories))  # Ensure it's character before splitting
  
  # Split multiple categories into separate rows
  category_counts <- data %>%
    separate_rows(Categories, sep = "; ") %>%  # Splitting category strings into individual rows
    count(Categories, sort = TRUE)
  
  categories <- unique(category_counts$Categories)
  palette <- setNames(viridis(length(categories), option = "D"), categories)
  
  # Bar chart for categorized topics
  ggplot(category_counts, aes(x = reorder(Categories, -n), y = n, fill = Categories)) +
